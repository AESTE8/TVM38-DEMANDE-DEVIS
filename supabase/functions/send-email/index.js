...
function renderMateriauxHtml(raw) {
    let data;
    try { data = JSON.parse(raw); } catch { return `<div style="font-family:monospace;font-size:12px;background:#f7f7f7;padding:12px 16px;border-radius:6px">${raw || 'Aucun matériau renseigné'}</div>`; }
    const palette = {
        livraison: { header: '#0053a1', bg: '#e3f0fb' },
        decharge:  { header: '#6a1b9a', bg: '#f3e5f5' },
        default:   { header: '#0053a1', bg: '#f0f4f8' },
    };
    const emojis = { livraison: '📦', decharge: '♻️' };
    let html = '';
    for (const section of data.sections) {
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        ...
        <tbody>${section.items.map((item, i) => `
        ...
        <tfoot><tr style="border-top:2px solid ${c.header};background:${c.bg}">
...
function buildHtml(fields) {
    const { prenom, nom, fonction, email, telephone, typeClient, dejaClient, entrepriseNom, entrepriseAdresse, agenceNom, typeDemande, adresseLivraison, camionLivraison, dateSouhaitee, creneau, materiauxData, notes } = fields;
    ...
    if (hasLivraison && adresseLivraison) {
        ...
        addressHtml = `<div style="margin-bottom:12px"><div style="font-size:11px;color:#888;margin-bottom:4px">Adresse chantier</div><div style="background:#f5f7fa;border:1px solid #dde3ea;border-radius:6px;padding:10px 14px;display:flex;justify-content:space-between;align-items:center;gap:12px"><span style="font-size:13px;font-weight:600;color:#111;flex:1">${adresseLivraison}</span><a href="${mapsUrl}" style="font-size:11px;color:#0053a1;text-decoration:none;white-space:nowrap;font-weight:bold" target="_blank">🗺️ Google Maps</a></div></div>`;
    } else if (typeDemande === 'fourniture' || typeDemande === 'decharge') {
        const lieLabel = typeDemande === 'fourniture' ? "Lieu d'enlèvement" : 'Lieu de dépôt';
        addressHtml = `<div style="margin-bottom:12px"><div style="font-size:11px;color:#888;margin-bottom:4px">${lieLabel}</div><div style="background:${tc.bg};border:1.5px solid ${tc.accent}40;border-radius:6px;padding:10px 14px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:13px;font-weight:600;color:${tc.accent}">Carrière TVM38</div><div style="font-size:11px;color:#666;margin-top:2px">${quarryAddr}</div></div><a href="${quarryMaps}" style="font-size:11px;color:${tc.accent};text-decoration:none;white-space:nowrap;font-weight:bold" target="_blank">🗺️ Google Maps</a></div></div>`;
    }
    const camionParts = camionLabel.split(' — ');
    const camionHtml = (hasLivraison && camionLabel) ? `<div style="margin-bottom:12px"><div style="font-size:11px;color:#888;margin-bottom:4px">Camion souhaité</div><div style="background:#f0f6ff;border:1.5px solid #b3d0f0;border-radius:6px;padding:10px 14px;display:flex;align-items:center;gap:12px"><span style="font-size:18px">🚛</span><div><div style="font-size:14px;font-weight:bold;color:#0053a1">${camionParts[0]}</div>${camionParts[1] ? `<div style="font-size:11px;color:#666;margin-top:2px">${camionParts[1]}</div>` : ''}</div></div></div>` : '';
...
${notes ? `<div class="card"><div class="slbl">Notes client</div><div class="notes-box">${notes.replace(/\n/g, '<br>')}</div></div>` : ''}
